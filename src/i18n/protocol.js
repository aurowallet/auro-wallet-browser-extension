const zh_CN = `Aura Wallet 是一款去中心化钱包。钱包中使用的助记词、私钥以及 Keystore 等加密数据全部由用户自己保管，您应自行承担妥善保管钱包设备、钱包密码、私钥、助记词 Keystore 等信息的责任。 

Auro Wallet 不负责为用户保管以上信息。因您遗失设备或硬件钱包、主动或被动泄露、遗忘钱包密码、私钥、助记词、Keystore 或遭受他人攻击、诈骗等所引起的一切风险、责任、损失、费用应由您自行承担。

请不要向任何人透漏您的助记词、私钥、Keystore、安全密码以及其他敏感信息，也不要将它们存放在任何云工具（如IM，邮件，社交网站等）。强烈建议使用硬件钱包与Mina Wallet链接，最大限度确保使用的安全性。

Aura Wallet 的代码还没有开源，正在接受审计机构的核查。用户需自行承担使用风险。

Aura Wallet 是社区开发的项目，不隶属于Mina Foundation 或者 O(1) Labs。`

const en = `Auro Wallet is a decentralized on-chain wallet. The encrypted data used in this wallet such as the Mnemonic Phrase, Private Key, and Keystore are all kept by Users. You shall bear sole responsibility to take care of your mobile devices, the wallet password, Private Key, Mnemonic Phrase and Keystore.
...
Aura Wallet code is not yet open source and is being reviewed by audit authority. Users shall assume their own risk of use.
