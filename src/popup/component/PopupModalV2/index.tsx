...
// CSS style type for styled-components interpolation
type CSSStyleObject = Record<string, string | number | undefined>;

interface StyledRightBtnWrapperProps {
  $rightBtnStyle?: RuleSet<object> | CSSStyleObject;
  $disabled?: boolean;
}

interface StyledLeftBtnWrapperDisabledProps {
  $disabled?: boolean;
}

interface PopupModalV2Props {
...
`;
const StyledRightBtnWrapper = styled(StyledBaseBtnWrapper)<StyledRightBtnWrapperProps>`
  color: #594af1;
...
const StyledLeftBtnWrapperDisabled = styled(StyledLeftBtnWrapper)<StyledLeftBtnWrapperDisabledProps>`
  ${(props) => props.$disabled && `
    opacity: 0.5;
    cursor: not-allowed;
    pointer-events: none;
