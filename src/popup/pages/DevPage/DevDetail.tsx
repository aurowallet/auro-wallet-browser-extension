...
const DevDetail = () => {
  ...
  const { fetchAccountData } = useFetchAccountData(currentAccount as Parameters<typeof useFetchAccountData>[0], true);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (pageType === "transaction") {
      ...
      setResponseBody(JSON.stringify(fullTxRequestResponse));
      setIsLoading(false);
    } else if (pageType === "pendingTx") {
      let pendingTxResponse = await getPendingTxList(
        currentAccount.address || ""
      ).catch((err) => err);
      setResponseBody(JSON.stringify(pendingTxResponse));
      setIsLoading(false);
    } else if (pageType === "pendingZkTx") {
      let zkAppPendingResponse = await getZkAppPendingTx(
        currentAccount.address || "",
        ZK_DEFAULT_TOKEN_ID as unknown as number
      ).catch((err) => err);
      setResponseBody(JSON.stringify(zkAppPendingResponse));
      setIsLoading(false);
    } else if (pageType === "balance") {
      let balanceResponse = await fetchAccountData().catch((err) => err);
      setResponseBody(JSON.stringify(balanceResponse));
      setIsLoading(false);
    }
