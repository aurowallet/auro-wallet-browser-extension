...
interface StyledTokenItemWrapperProps {
  $newToken?: boolean;
}

const StyledTokenItemWrapper = styled.div<StyledTokenItemWrapperProps>`
  display: flex;
  align-items: center;
...
const StyledTokenAmount = styled.div`
  ...
  font-weight: 400;
`;
interface TokenManageItemProps {
  token: {
    tokenId: string;
    tokenBaseInfo: {
      isMainToken: boolean;
...
const TokenManageItem = ({ token }: TokenManageItemProps) => {
  ...
  const {
    ...
    if (isFungibleToken) {
      tokenSymbol =
        (token?.tokenNetInfo?.tokenSymbol?.length || 0) > 0
