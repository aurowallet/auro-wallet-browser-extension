...
const Wallet = ({}) => {
  ...
  const checkLocalWatchWallet = useCallback(() => {
    sendMsg(
      ...
      (account: { accounts: { watchList: unknown[] } }) => {
...
`;
interface StyledCurrencyRowProps {
  $isCache?: boolean;
}

const StyledCurrencyRow = styled.div<StyledCurrencyRowProps>`
  font-weight: 700;
  font-size: 32px;
  color: #ffffff;
  margin-top: 20px;
...
`
interface StyledReceiveBtnProps extends StyledBaseBtnProps {
  $showStaking?: boolean;
}
const StyledReceiveBtn = styled(StyledBaseBtn)<StyledReceiveBtnProps>`
  &:hover {
    background: #f2f2f2;
...
const WalletInfo = () => {
  ...
  const tokenTotalAmount = useAppSelector(
