...
export const RestoreMneView = ({
  ...
  const [mneInputList, setMneInputList] = useState<string[]>(Array(12).fill(""));
  ...
  const onChangeMneItem = useCallback(
    (e: InputChangeEvent, index: number) => {
  ...
  const handleImportSuccess = useCallback(
    (account: AccountInfo & { error?: string; type?: string; existingAccount?: AccountInfo }) => {
      setBtnLoading(false);
      if (account.error) {
